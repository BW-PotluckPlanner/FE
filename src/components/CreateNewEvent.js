...
function CreateNewEvent(props) {
  ...
  const setData = () =>{
      ...
      console.log({"name": date.title, "date": date.date, "time_start": date.time, "time_end": date.end, "description": date.description, "userId": id})
      axiosWithAuth().post('api/potluck', {"name": date.title, "date": date.date, "time_start": date.time, "time_end": date.end, "description": date.description, "userId": id}).then(res =>{
